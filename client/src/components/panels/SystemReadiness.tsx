        ...
        {Object.keys(status).sort().map((key) => (
          <div key={key} className="flex flex-col gap-1 border-b border-houston-muted/30 pb-2">
            <div className="flex justify-between text-[10px]">
              ...
              <span className={cn(
                "font-bold",
                status[key] === 'GO' ? "text-houston-green" : status[key] === 'NO-GO' ? "text-red-500" : "text-yellow-500"
              )}>{status[key]}</span>
            </div>
            <div className="flex gap-2">
              <button
                ...
                className={cn(
                  ...
                  status[key] === 'GO' ? "bg-houston-green text-black border-houston-green" : "border-houston-muted hover:border-houston-green"
                )}
              >
                GO
              </button>
              <button
                ...
                className={cn(
                  "flex-1 text-[9px] py-1 border transition-all",
                  status[key] === 'NO-GO' ? "bg-red-500 text-black border-red-500" : "border-houston-muted hover:border-red-500"
                )}
              >
                NO-GO
              </button>
